...
/***/ function(module, exports, __webpack_require__) {
	...
	function retnFunc(retn) {
	    ...
	    if (resmsg == 'B00002') { //表示该手机号已开某通联卡，但是未有绑定微信号
	        ...
	        window.location.href = BASE_URL + "FuLiBao/protocol_members.html?" + paramStr + "&jm_openid=" + openid + "&openid=" + des_openid;
	        return;
	    }
...
/***/ function(module, exports) {
	...
	function spaceCardNum() {
	    ...
	    if (needCardVal.length > 4 && needCardVal.length <= 8) {
	        creditCardVal = needCardVal.substr(0, 4) + "  " + needCardVal.substr(4);

	    } else if (needCardVal.length > 8 && needCardVal.length <= 12) {
	        creditCardVal = needCardVal.substr(0, 4) + "  " + needCardVal.substr(4, 4) + "  " + needCardVal.substr(8);
	    } else if (needCardVal.length > 12 && needCardVal.length <= 16) {
...
/***/ function(module, exports) {
	...
	function setConfig(msg) {
	    wx.config({
	        debug: false,
	        appId: 'wx69be6f067db3c95a', //'wxf8b4f85f3a794e77',
