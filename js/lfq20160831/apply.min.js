...
/***/ function(module, exports, __webpack_require__) {
	...
	var cardFeeRate6 = 0.00; // 过渡用改成下拉选项后没用
	var cardFeeRate12 = 0.00; // 过渡用改成下拉选项后没用
	$(function() {
	...
	function setContractHref() {
	    var theHref = "contract.html?openid=" + openid + "&cardFeeRate=" + cardFeeRate + "&merId=" + merId + "&merName=" + merName + "&lfq_cnt=" + stageNumber;
	...
	function getParameter() {
	    ...
	    wl.postAjax(LFQ_ORDER_CREATE_URL, param, function(retn) {
	        console.log(retn);
	        if (retn.lfq_merinfo_response) {
	            ...
	            if (merinfo.mer_prdt_rate_arrays) { // 过渡先取6期12期
	                var rateArrays = merinfo.mer_prdt_rate_arrays.mer_prdt_rate;
	                for (var i = 0, l = rateArrays.length; i < l; i++) {
	                    if (rateArrays[i].stageCount == "6") {
	                        cardFeeRate6 = rateArrays[i].custRate;
	                        cardFeeRate = cardFeeRate6;
	                        prdt_no = rateArrays[i].prdtNo;
	                    }
	                    if (rateArrays[i].stageCount == "12") {
	                        cardFeeRate12 = rateArrays[i].custRate;
	                        prdt_no = rateArrays[i].prdtNo;
	                    }
	                }
	            }
	...
	// 初始化页面
	function inint() {
	    maxApply = 20000 / (1 + cardFeeRate / 100);
	    $("#maxApplyNum").text(Math.floor(maxApply));
	    $("#applyInstalmentNum").attr("placeholder", "目前只支持600-" + Math.floor(maxApply));
	...
	// 根据选择期数改变费率
	function setCardFeeRate() {
	    if (stageNumber == 6) {
	        cardFeeRate = cardFeeRate6;
	    } else {
	        cardFeeRate = cardFeeRate12;
	    }

	}

	// 设置选择分期的样式
	function setStageNumClass($parentNode) {
	...
	function setEachFee() {
	    ...
	    $("#eachFee").text(eachFee.toFixed(2) + '%');
	}

	// 提示是否在允许的申请金额内
	function allowApplyNum() {

	    if (applyNum < 600 || applyNum > maxApply) {
	        isAllow = false;
	...
	function isDisabledNext(argument) {
	    ...
	    if (agreementValue && isAllow) {
	        $("#next-step").attr("class", "btn btn_primary");
	    } else {
	        $("#next-step").attr("class", "btn btn_disabled btn_default");
	    }
	}
	...
	function nextStepFunc() {
	    var agreementValue = $("#agreement-check").is(":checked");
	    if (agreementValue && isAllow) {
	        window.location.href = "applyNextStep.html?lfq_amt=" + applyNum + "&lfq_cnt=" + stageNumber + "&openid=" + openid + "&merId=" + merId + "&cardFeeRate=" + cardFeeRate + "&merName=" + merName + "&prdt_no=" + prdt_no;
	    }
	}
